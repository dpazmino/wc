# Round of 32 — Match Predictions

*Strength model (shrink 0.25) **anchored 50/50 to the de-vigged DraftKings closing moneyline** where odds are available (rows marked *model-only* have none yet). Knockout metric is **P(advance) = P(win) + ½·P(draw)** — a level game is treated as a coin-flip shootout. Host nations (USA, Mexico, Canada) get Elo home-field advantage when they play; all other ties are neutral. W/D/L = first team's Win / Draw / Loss % over 90 minutes (post-anchor).*

## Summary

| Match | Date (ET) | W/D/L | xG | To advance | Verdict | Result |
|---|---|---|---|---|---|---|
| South Africa vs Canada *(host Canada at home)* | Sat Jun 28, 3:00 PM | 22 / 25 / 53 | 1.0–1.6 | **Canada 66%** | Canada favoured | **Canada** ✅ hit (0–1) |
| Brazil vs Japan | Mon Jun 29, 1:00 PM | 54 / 24 / 22 | 1.6–1.0 | **Brazil 66%** | Brazil favoured | **Brazil** ✅ hit (2–1) |
| Germany vs Paraguay | Mon Jun 29, 4:30 PM | 66 / 19 / 15 | 1.7–0.9 | **Germany 76%** | Germany strong | **Paraguay** ❌ miss (1–1, pens) |
| Netherlands vs Morocco | Mon Jun 29, 9:00 PM | 46 / 28 / 26 | 1.5–1.1 | **Netherlands 60%** | Slight edge Netherlands | **Morocco** ❌ miss (1–1, pens) |
| Ivory Coast vs Norway | Tue Jun 30, 1:00 PM | 30 / 28 / 42 | 1.3–1.3 | **Norway 56%** | Slight edge Norway | — |
| France vs Sweden | Tue Jun 30, 5:00 PM | 64 / 19 / 17 | 1.7–0.9 | **France 74%** | France favoured | — |
| Mexico vs Ecuador *(host Mexico at home)* | Tue Jun 30, 9:00 PM | 46 / 25 / 29 | 1.5–1.1 | **Mexico 59%** | Slight edge Mexico · *model-only* | — |
| England vs DR Congo | Wed Jul 1, 12:00 PM | 63 / 19 / 18 | 1.7–0.9 | **England 72%** | England favoured · *model-only* | — |
| Belgium vs Senegal | Wed Jul 1, 4:00 PM | 48 / 25 / 27 | 1.5–1.1 | **Belgium 61%** | Slight edge Belgium · *model-only* | — |
| USA vs Bosnia and Herzegovina *(host USA at home)* | Wed Jul 1, 8:00 PM | 60 / 22 / 18 | 1.5–1.1 | **USA 71%** | USA favoured | — |
| Spain vs Austria | Thu Jul 2, 3:00 PM | 51 / 23 / 26 | 1.6–1.0 | **Spain 63%** | Slight edge Spain · *model-only* | — |
| Portugal vs Croatia | Thu Jul 2, 7:00 PM | 48 / 25 / 27 | 1.5–1.1 | **Portugal 60%** | Slight edge Portugal · *model-only* | — |
| Switzerland vs Algeria | Thu Jul 2, 11:00 PM | 44 / 26 / 30 | 1.5–1.1 | **Switzerland 57%** | Slight edge Switzerland · *model-only* | — |
| Australia vs Egypt | Fri Jul 3, 2:00 PM | 28 / 29 / 43 | 1.1–1.5 | **Egypt 57%** | Slight edge Egypt | — |
| Argentina vs Cape Verde | Fri Jul 3, 6:00 PM | 74 / 15 / 11 | 1.9–0.7 | **Argentina 82%** | Argentina strong | — |
| Colombia vs Ghana | Fri Jul 3, 9:30 PM | 41 / 27 / 32 | 1.5–1.1 | **Colombia 55%** | Toss-up · *model-only* | — |

**Model on advancement so far: 2/4 correct (50%)** across the 4 decided tie(s) of 16. ✅ = the model's P(advance) favourite went through; ❌ = it was knocked out (a shootout loss by the favourite counts as a miss). Grade is on **advancement**, not the 90' score.

## Detail

### South Africa vs Canada — Sat Jun 28, 3:00 PM  *(host Canada at home)*
- Model+market **21.7 / 25.0 / 53.3** (W/D/L) · xG 1.04–1.56.
- **To advance:** South Africa 34% · Canada 66% → **Canada**.  *(model-only had South Africa 37%; market trimmed it.)*
- **Result:** **Canada** advanced (0–1) — model ✅ correct.

### Brazil vs Japan — Mon Jun 29, 1:00 PM
- Model+market **53.8 / 23.7 / 22.5** (W/D/L) · xG 1.60–1.00.
- **To advance:** Brazil 66% · Japan 34% → **Brazil**.  *(model-only had Brazil 63%; market lifted it.)*
- **Result:** **Brazil** advanced (2–1) — model ✅ correct.

### Germany vs Paraguay — Mon Jun 29, 4:30 PM
- Model+market **66.4 / 19.0 / 14.6** (W/D/L) · xG 1.73–0.87.
- **To advance:** Germany 76% · Paraguay 24% → **Germany**.  *(model-only had Germany 73%; market lifted it.)*
- **Result:** **Paraguay** advanced (1–1 after penalties) — model ❌ wrong (favourite eliminated).

### Netherlands vs Morocco — Mon Jun 29, 9:00 PM
- Model+market **46.0 / 27.5 / 26.4** (W/D/L) · xG 1.49–1.11.
- **To advance:** Netherlands 60% · Morocco 40% → **Netherlands**.  *(model-only had Netherlands 59%; market lifted it.)*
- **Result:** **Morocco** advanced (1–1 after penalties) — model ❌ wrong (favourite eliminated).

### Ivory Coast vs Norway — Tue Jun 30, 1:00 PM
- Model+market **29.7 / 28.0 / 42.4** (W/D/L) · xG 1.27–1.33.
- **To advance:** Ivory Coast 44% · Norway 56% → **Norway**.  *(model-only had Ivory Coast 48%; market trimmed it.)*

### France vs Sweden — Tue Jun 30, 5:00 PM
- Model+market **63.9 / 19.4 / 16.7** (W/D/L) · xG 1.73–0.87.
- **To advance:** France 74% · Sweden 26% → **France**.  *(model-only had France 65%; market lifted it.)*

### Mexico vs Ecuador — Tue Jun 30, 9:00 PM  *(host Mexico at home)*
- Model **46.1 / 25.4 / 28.5** (W/D/L) · xG 1.52–1.08.
- **To advance:** Mexico 59% · Ecuador 41% → **Mexico**.

### England vs DR Congo — Wed Jul 1, 12:00 PM
- Model **62.5 / 19.1 / 18.4** (W/D/L) · xG 1.74–0.86.
- **To advance:** England 72% · DR Congo 28% → **England**.

### Belgium vs Senegal — Wed Jul 1, 4:00 PM
- Model **48.4 / 24.7 / 26.9** (W/D/L) · xG 1.51–1.09.
- **To advance:** Belgium 61% · Senegal 39% → **Belgium**.

### USA vs Bosnia and Herzegovina — Wed Jul 1, 8:00 PM  *(host USA at home)*
- Model+market **59.9 / 21.7 / 18.5** (W/D/L) · xG 1.47–1.13.
- **To advance:** USA 71% · Bosnia and Herzegovina 29% → **USA**.  *(model-only had USA 62%; market lifted it.)*

### Spain vs Austria — Thu Jul 2, 3:00 PM
- Model **51.1 / 22.9 / 26.0** (W/D/L) · xG 1.62–0.98.
- **To advance:** Spain 63% · Austria 37% → **Spain**.

### Portugal vs Croatia — Thu Jul 2, 7:00 PM
- Model **47.6 / 24.9 / 27.5** (W/D/L) · xG 1.53–1.07.
- **To advance:** Portugal 60% · Croatia 40% → **Portugal**.

### Switzerland vs Algeria — Thu Jul 2, 11:00 PM
- Model **44.4 / 25.7 / 30.0** (W/D/L) · xG 1.54–1.06.
- **To advance:** Switzerland 57% · Algeria 43% → **Switzerland**.

### Australia vs Egypt — Fri Jul 3, 2:00 PM
- Model+market **28.4 / 28.7 / 42.9** (W/D/L) · xG 1.12–1.48.
- **To advance:** Australia 43% · Egypt 57% → **Egypt**.  *(model-only had Australia 40%; market lifted it.)*

### Argentina vs Cape Verde — Fri Jul 3, 6:00 PM
- Model+market **74.1 / 15.0 / 10.9** (W/D/L) · xG 1.87–0.73.
- **To advance:** Argentina 82% · Cape Verde 18% → **Argentina**.  *(model-only had Argentina 74%; market lifted it.)*

### Colombia vs Ghana — Fri Jul 3, 9:30 PM
- Model **41.0 / 27.4 / 31.6** (W/D/L) · xG 1.49–1.11.
- **To advance:** Colombia 55% · Ghana 45% → **Colombia**.

*Anchored 50/50 to DraftKings where odds exist (`apply_odds.HIST`). Regenerate: `python gen_ko_preds.py r32`. Log results to `data/results.csv` after kickoff (knockout: log the score for Elo, track the advancer separately).*
